...
export async function POST(request: NextRequest) {
  try {
    ...
    const maxSizeBytes = parseInt(process.env.MAX_PROOF_UPLOAD_BYTES || '10485760'); // 10MB
    ...
    if (!isValidProofContentType(contentType)) {
      return NextResponse.json(
        { error: 'Invalid file type. Only images (JPEG, PNG, GIF) and PDF files are allowed for payment proofs.' },
        { status: 400 }
      );
    }
