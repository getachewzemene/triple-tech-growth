...
export const validateURL = (url: string): { isValid: boolean; message: string; sanitized?: string } => {
...
export const validateFile = (file: File, allowedTypes: string[], maxSize: number): { isValid: boolean; message: string } => {
  ...
  if (!allowedTypes.some(type => fileType.includes(type) || fileName.endsWith(type))) {
  ...
  if (file.size > maxSize) {
    ...
    return { isValid: false, message: `File too large. Maximum size: ${maxSizeMB}MB` };
  }
  
  // Check for suspicious file names
  const suspiciousPatterns = ['.exe', '.bat', '.cmd', '.scr', '.js', '.vbs', '.jar'];
...
export const checkRateLimit = (key: string, maxAttempts: number, timeWindow: number): { isAllowed: boolean; remainingAttempts: number } => {
  const now = Date.now();
  ...
  try {
    ...
    const validAttempts = attempts.filter((timestamp: number) => now - timestamp < timeWindow);
    ...
    return { isAllowed: true, remainingAttempts: maxAttempts - validAttempts.length };
  } catch (error) {
...
// Validate form token
export const validateFormToken = (token: string, storedToken: string): boolean => {
