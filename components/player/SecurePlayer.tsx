 ...
 */
export default function SecurePlayer({ courseId, onPlayerReady, onError }: SecurePlayerProps) {
  const { user } = useAuth();
  ...
  const loadShakaPlayer = useCallback(async () => {
    ...
    script.src = 'https://cdnjs.cloudflare.com/ajax/libs/shaka-player/4.7.5/shaka-player.compiled.js';
    script.async = true;
   ...
   */
  const requestPlaybackToken = useCallback(async (): Promise<PlaybackSession | null> => {
    try {
   ...
   */
  const getManifestUrl = useCallback(async (token: string): Promise<string | null> => {
    try {
      const response = await fetch(`/api/player/manifest?courseId=${courseId}`, {
        headers: {
   ...
   */
  const initializePlayer = useCallback(async (manifestUrl: string) => {
    if (!videoRef.current) {
      ...
      player.configure({
        drm: {
          servers: {
            ...
            'com.microsoft.playready': process.env.NEXT_PUBLIC_PLAYREADY_LICENSE_URL || '',
            'com.apple.fps.1_0': process.env.NEXT_PUBLIC_FAIRPLAY_LICENSE_URL || '',
          },
          advanced: {
            'com.widevine.alpha': {
  ...
  const handlePlayerError = useCallback((event: any) => {
    ...
    if (error.code === shaka.util.Error.Code.LICENSE_REQUEST_FAILED) {
      errorMessage = 'DRM license request failed. Please check your connection and try again.';
    } else if (error.code === shaka.util.Error.Code.MANIFEST_REQUEST_FAILED) {
      errorMessage = 'Video manifest request failed. Please refresh and try again.';
    } else if (error.code === shaka.util.Error.Code.SEGMENT_REQUEST_FAILED) {
      errorMessage = 'Video segment request failed. Please check your connection.';
    }
  ...
  }, [requestPlaybackToken, getManifestUrl, initializePlayer, updateWatermark, onError, toast]);
      ...
      >
        <div className="bg-black bg-opacity-30 text-white text-xs px-2 py-1 rounded backdrop-blur-sm flex items-center space-x-2">
          ...
          <div className="flex flex-col">
            <span className="font-semibold text-white opacity-70">TripleAcademy</span>
            <span className="text-white opacity-60">{formatUserIdentifier()}</span>
