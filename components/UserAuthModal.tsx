...
const validatePassword = (password: string): { isValid: boolean; message: string; strength: number } => {
  ...
  if (password.length < 8) return { isValid: false, message: 'Password must be at least 8 characters long', strength: 1 };
  ...
  if (strength < 3) return { isValid: false, message: 'Password must contain uppercase, lowercase, and number', strength };
...
const SecurityIndicator = ({ level }: { level: number }) => {
  ...
  return (
    <div className="flex items-center gap-2 mt-1">
      ...
      <span className={`text-xs font-medium ${getColor()}`}>{getMessage()}</span>
      <div className="flex gap-1 ml-auto">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className={`w-2 h-2 rounded-full ${i <= level ? getColor().replace('text-', 'bg-') : 'bg-gray-200'}`} />
        ))}
      </div>
    </div>
  );
...
const UserAuthModal = ({ isOpen, onClose }: UserAuthModalProps) => {
  ...
  const validateSignupField = (field: string, value: string) => {
    ...
    switch (field) {
      ...
      case 'password':
        ...
        newErrors.password = passwordValidation.isValid ? '' : passwordValidation.message;
        setPasswordStrength(passwordValidation.strength);
        
        // Re-validate confirm password if it exists
        if (signupData.confirmPassword) {
          newErrors.confirmPassword = value === signupData.confirmPassword ? '' : 'Passwords do not match';
        }
        break;
      ...
      case 'phone':
        ...
        newErrors.phone = phoneValidation.isValid ? '' : phoneValidation.message;
        break;
    }
  ...
  const handleForgotPassword = (e: React.FormEvent) => {
    ...
    setForgotErrors({ email: emailValidation.isValid ? '' : emailValidation.message });
              ...
              <div className="relative">
                ...
                {forgotErrors.email && forgotEmail && (
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-red-500">{forgotErrors.email}</span>
                  </div>
                )}
                {!forgotErrors.email && forgotEmail && validateEmail(forgotEmail).isValid && (
                  <div className="flex items-center gap-1 mt-1">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  {loginErrors.email && loginData.email && (
                    <div className="flex items-center gap-1 mt-1">
                      <AlertTriangle className="h-3 w-3 text-red-500" />
                      <span className="text-xs text-red-500">{loginErrors.email}</span>
                    </div>
                  )}
                  {!loginErrors.email && loginData.email && validateEmail(loginData.email).isValid && (
                    <div className="flex items-center gap-1 mt-1">
                      <CheckCircle2 className="h-3 w-3 text-green-500" />
                      <span className="text-xs text-green-500">Valid email</span>
                    </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  {loginErrors.password && loginData.password && (
                    <div className="flex items-center gap-1 mt-1">
                      <AlertTriangle className="h-3 w-3 text-red-500" />
                      <span className="text-xs text-red-500">{loginErrors.password}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  {signupErrors.fullName && signupData.fullName && (
                    <div className="flex items-center gap-1 mt-1">
                      <AlertTriangle className="h-3 w-3 text-red-500" />
                      <span className="text-xs text-red-500">{signupErrors.fullName}</span>
                    </div>
                  )}
                  {!signupErrors.fullName && signupData.fullName && signupData.fullName.length >= 2 && (
                    <div className="flex items-center gap-1 mt-1">
                      <CheckCircle2 className="h-3 w-3 text-green-500" />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  {signupErrors.email && signupData.email && (
                    <div className="flex items-center gap-1 mt-1">
                      <AlertTriangle className="h-3 w-3 text-red-500" />
                      <span className="text-xs text-red-500">{signupErrors.email}</span>
                    </div>
                  )}
                  {!signupErrors.email && signupData.email && validateEmail(signupData.email).isValid && (
                    <div className="flex items-center gap-1 mt-1">
                      <CheckCircle2 className="h-3 w-3 text-green-500" />
                      <span className="text-xs text-green-500">Valid email</span>
                    </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  {signupErrors.phone && signupData.phone && (
                    <div className="flex items-center gap-1 mt-1">
                      <AlertTriangle className="h-3 w-3 text-red-500" />
                      <span className="text-xs text-red-500">{signupErrors.phone}</span>
                    </div>
                  )}
                  {!signupErrors.phone && signupData.phone && validatePhone(signupData.phone).isValid && (
                    <div className="flex items-center gap-1 mt-1">
                      <CheckCircle2 className="h-3 w-3 text-green-500" />
              ...
              <div className="space-y-2">
                ...
                {signupErrors.password && signupData.password && (
                  <div className="flex items-center gap-1 mt-1">
                    <AlertTriangle className="h-3 w-3 text-red-500" />
                    <span className="text-xs text-red-500">{signupErrors.password}</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {signupErrors.confirmPassword && signupData.confirmPassword && (
                  <div className="flex items-center gap-1 mt-1">
                    <AlertTriangle className="h-3 w-3 text-red-500" />
                    <span className="text-xs text-red-500">{signupErrors.confirmPassword}</span>
                  </div>
                )}
                {!signupErrors.confirmPassword && signupData.confirmPassword && signupData.password === signupData.confirmPassword && signupData.password && (
                  <div className="flex items-center gap-1 mt-1">
                    <CheckCircle2 className="h-3 w-3 text-green-500" />
